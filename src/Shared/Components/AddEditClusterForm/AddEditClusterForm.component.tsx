...
import { ReactComponent as ErrorIcon } from '../../../Assets/Icon/ic-warning-y6.svg'
import NoResults from '../../../Assets/Img/empty-noresult@2x.png'
import { saveCluster, updateCluster, deleteCluster, validateCluster, saveClusters } from './service'
import { ReactComponent as Close } from '../../../Assets/Icon/ic-close.svg'
import { ReactComponent as Warning } from '../../../Assets/Icon/ic-alert-triangle.svg'
...
import { ReactComponent as ForwardArrow } from '../../../Assets/Icon/ic-arrow-right.svg'
import { ReactComponent as MechanicalOperation } from '../../../Assets/Img/ic-mechanical-operation.svg'
import { DataListType, UserDetails, SaveClusterPayloadType, SSHAuthenticationType } from './types'
import { AuthenticationType, RemoteConnectionTypeCluster, CLUSTER_COMMAND } from './constants'
...
const AddClusterHeader = ({ id, handleCloseButton }: { id: number; handleCloseButton: () => void }) => (
    <div className="flex flex-align-center dc__border-bottom flex-justify bcn-0 pb-12 pt-12 pl-20 pr-20">
        <h2 data-testid="add_cluster_header" className="fs-16 fw-6 lh-1-43 m-0 title-padding">
            <span className="fw-6 fs-16 cn-9">{id ? 'Edit Cluster' : 'Add Cluster'}</span>
        </h2>
        <button
...
const LoadingCluster = ({ id, handleCloseButton }: { id: number; handleCloseButton: () => void }): JSX.Element => (
    <div className="cluster-form dc__position-rel h-100 bcn-0">
        <AddClusterHeader id={id} handleCloseButton={handleCloseButton} />
        ...
        <div className="w-100 dc__border-top flex right pb-12 pt-12 pr-20 pl-20 dc__position-fixed dc__position-abs ">
            ...
            <button className="cta ml-12 h-36 lh-36" disabled aria-label="loading" type="button">
                <Progressing />
            </button>
        </div>
    </div>
...
const AddEditClusterForm = ({
    ...
    const [, grafanaModuleStatus] = useAsync(() => getModuleInfo(ModuleNameMap.GRAFANA), [], !window._env_.K8S_CLIENT)
    ...
    const getClusterPayload = (payloadState) => ({
        id,
        insecureSkipTlsVerify: !isTlsConnection,
        cluster_name: payloadState.cluster_name.value,
        config: {
            bearer_token:
                payloadState.token.value && payloadState.token.value !== DEFAULT_SECRET_PLACEHOLDER
                    ? payloadState.token.value
                    : '',
            tls_key: payloadState.tlsClientKey.value,
            cert_data: payloadState.tlsClientCert.value,
            cert_auth_data: payloadState.certificateAuthorityData.value,
        },
        active,
        remoteConnectionConfig: getRemoteConnectionConfig(payloadState, remoteConnectionMethod, SSHConnectionType),
        prometheus_url: prometheusToggleEnabled ? payloadState.endpoint.value : '',
        prometheusAuth: {
            userName:
                prometheusToggleEnabled && payloadState.authType.value === AuthenticationType.BASIC
                    ? payloadState.userName.value
                    : '',
            password:
                prometheusToggleEnabled && payloadState.authType.value === AuthenticationType.BASIC
                    ? payloadState.password.value
                    : '',
            tlsClientKey: prometheusToggleEnabled ? payloadState.prometheusTlsClientKey.value : '',
            tlsClientCert: prometheusToggleEnabled ? payloadState.prometheusTlsClientCert.value : '',
            isAnonymous: payloadState.authType.value === AuthenticationType.ANONYMOUS,
        },
        // TODO Revisit
    ...
    const onValidation = async (stateToValidate) => {
        const payload = getClusterPayload(stateToValidate)
        const urlValue = stateToValidate.url.value?.trim() ?? ''
        ...
        if (remoteConnectionMethod === RemoteConnectionType.Proxy) {
            let proxyUrlValue = stateToValidate.proxyUrl?.value?.trim() ?? ''
        ...
        if (stateToValidate.authType.value === AuthenticationType.BASIC && prometheusToggleEnabled) {
            const isValid = stateToValidate.userName?.value && stateToValidate.password?.value
            if (!isValid) {
                toast.error('Please add both username and password')
                return
            }
            payload.prometheusAuth.userName = stateToValidate.userName.value || ''
            payload.prometheusAuth.password = stateToValidate.password.value || ''
            payload.prometheusAuth.tlsClientKey = stateToValidate.prometheusTlsClientKey.value || ''
            payload.prometheusAuth.tlsClientCert = stateToValidate.prometheusTlsClientCert.value || ''
        }
        if (isTlsConnection) {
            if (
                stateToValidate.tlsClientKey.value ||
                stateToValidate.tlsClientCert.value ||
                stateToValidate.certificateAuthorityData.value
            ) {
                payload.config.tls_key = stateToValidate.tlsClientKey.value || ''
                payload.config.cert_data = stateToValidate.tlsClientCert.value || ''
                payload.config.cert_auth_data = stateToValidate.certificateAuthorityData.value || ''
            }
        }
    ...
    const getSaveClusterPayload = (dataLists: DataListType[]) => {
        const saveClusterPayload: SaveClusterPayloadType[] = dataLists
            .filter((_dataList) => isClusterSelected[_dataList.cluster_name])
            .map<SaveClusterPayloadType>((_dataList) => ({
                id: _dataList.id,
    ...
    function YAMLtoJSON(yamlToConvert) {
        try {
            const obj = YAML.parse(yamlToConvert)
    ...
    async function validateClusterDetail() {
        try {
            ...
            await validateCluster(payload).then((response) => {
                ...
                setDataList([
                    ...Object.values(response.result).map((_cluster: any) => {
    ...
    const renderUrlAndBearerToken = () => {
        ...
        return (
            <>
                ...
                {!isDefaultCluster && RemoteConnectionRadio && (
                    <>
                        <hr />
                ...
                {isGrafanaModuleInstalled && (
                    <>
                        <div className={`${prometheusToggleEnabled || prometheusUrl ? 'mb-20' : 'mb-40'} mt-20`}>
                            ...
                            <div>
                                ...
                                <div className="form__row">
                                    ...
                                    >
                                        ...
                                        <RadioGroupItem value={AuthenticationType.ANONYMOUS}>
                                            {' '}
                                            Anonymous{' '}
                                        </RadioGroupItem>
                                    </RadioGroup>
    ...
    if (loader) {
        return <LoadingCluster id={id} handleCloseButton={handleCloseButton} />
    }
            ...
            <div className="api-token__list en-2 bw-0 bcn-0 br-8">
                ...
                <div className="dc__overflow-scroll" style={{ height: 'calc(100vh - 161px)' }}>
                    {!saveClusterList || saveClusterList.length === 0 ? (
                        ...
                        saveClusterList.map((clusterListDetail) => (
                            <div
                                key={`api_${clusterListDetail.clusterName}`}
    ...
    function toggleIsSelected(toggleClusterName: string, forceUnselect?: boolean) {
        const _currentSelections = {
            ...isClusterSelected,
            [toggleClusterName]: forceUnselect ? false : !isClusterSelected[toggleClusterName],
        }
                            ...
                            <div className="api-token__list en-2 bw-1 bcn-0 br-4 mr-20 ml-20 mt-16">
                                ...
                                <div style={{ height: 'auto' }}>
                                    {!dataList || dataList.length === 0 ? (
                                        ...
                                        dataList.map((clusterDetail) => (
                                            <div
                                                key={`api_${clusterDetail.id}`}
