...
import { AppEnvDeploymentConfigDTO, AppEnvDeploymentConfigType, TemplateListDTO } from '@Shared/Services'
...
export const getEnvironmentIdByEnvironmentName = (environments: EnvironmentOptionType[], name: string) =>
    environments.find(({ name: _name }) => name === _name)?.id ?? APP_CONFIG_BASE_CONFIGURATIONS.id
        ...
        : []),
    {
        ...
        options: previousDeploymentsList.map(
            ({ finishedOn, chartVersion, pipelineId, deploymentTemplateHistoryId, chartRefId }) => ({
                label: `${moment(finishedOn).format(DATE_TIME_FORMATS.TWELVE_HOURS_FORMAT)} (v${chartVersion})`,
